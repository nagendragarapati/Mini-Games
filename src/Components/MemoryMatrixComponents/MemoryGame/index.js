...
const MemoryGame = () => {
    ...
    let splicedRandomNums = randomNums?.slice(0, N)
    let reArrangedBoxes = boxarray?.map(box => {
    ...
    const onClickBox = (boxId) => {
        ...
        if (clickedWrongBoxes.length === 0) {
            if (numIncludes) {
                newBoxesArray = boxarray.map(obj => obj.id === boxId ? { ...obj, isActive: true } : obj)
                setClickedBoxes([...clickedSuccessBoxes, boxId])

            }
            else {
                newBoxesArray = boxarray.map(obj => obj.id === boxId ? { ...obj, wrongClicked: true } : obj)
    ...
    return (
        <div className='matrix-main'>
            ...
            <div className="back-rule-header">
                <div className="back-container" onClick={() => navigate('/memory-game-rules')}>
                ...
                <RulesModal
                    ...
                    rulesset1={memoryRuleSet1} rulesset2={memoryRuleSet2}
                />
                    ...
                    <div className='boxes-middle-container'>
                        {
                            ...
                            timeRunning ? <ul className='boxes-inner-container'>
                                {

                                    reArrangedBoxes?.map(each => <li className={each.isActive ? 'box active-box' : 'box'} style={{ width: boxWidth, height: boxWidth }} id={`box${each.id}`}></li>)
                                }

                            </ul> : <ul className='boxes-inner-container'>
                                {

                                    boxarray?.map(each => <li className={getBoxClassName(each)} style={{ width: boxWidth, height: boxWidth }} id={`box${each.id}`} onClick={() => onClickBox(each.id)}></li>)
                                }
